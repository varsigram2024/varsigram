...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        const profileResponse = await axios.get(`https://api.varsigram.com/api/v1/users/${username}/`, {
          headers: {
        ...
        if (user?.email !== profileResponse.data.email) {
          const followingResponse = await axios.get(`https://api.varsigram.com/api/v1/users/${username}/following/`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
          ...
          const isUserFollowing = followingList.some((followedUser: any) => 
            followedUser.email === profileResponse.data.email
          );
          setIsFollowing(isUserFollowing);
        }

        // Fetch user's posts
        const postsResponse = await axios.get(`https://api.varsigram.com/api/v1/posts/user/${profileResponse.data.id}/`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
  ...
  const handleFollow = async () => {
    ...
    try {
      const usernameParam = username.split('@')[0];
      
      if (isFollowing) {
        await axios.delete(`https://api.varsigram.com/api/v1/users/${usernameParam}/unfollow/`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
        ...
        await axios.post(`https://api.varsigram.com/api/v1/users/${usernameParam}/follow/`, {}, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
  ...
  return (
    <div className="flex flex-col items-center justify-start w-full bg-gray-100">
      <div className="flex w-full items-start justify-center bg-white">
        <Sidebar1 onComplete={handleNavigation} currentPage="profile" />
                ...
                <div className="mx-3.5 ml-4 flex flex-col items-start gap-4">
                  ...
                  <div className="flex flex-wrap gap-6">
                    <Text as="p" className="text-[16px] font-normal">
                      <span className="font-semibold">{userProfile.followers_count || 0}</span> Followers
                    </Text>
                    <Text as="p" className="text-[16px] font-normal">
                      <span className="font-semibold">{userProfile.following_count || 0}</span> Following
                    </Text>
                  </div>
        ...
        <BottomNav onComplete={handleNavigation} currentPage="profile" />
      </div>
    </div>
  );
}
